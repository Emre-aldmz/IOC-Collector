...
def create_parser():
    ...
    input_group.add_argument(
        "--cert-feed",
        help="CERT feed çek (ülke kodu: TR, US, LU veya 'all')"
    )
    input_group.add_argument(
        "--github-feed",
    ...
    input_group.add_argument(
        "--github-feed-preset",
        help="Bilinen GitHub IOC repo preset'i"
    )
    input_group.add_argument(
        "--add-feed",
        nargs=2,
...
def main():
    ...
    try:
        ...
        if args.cert_feed:
            ...
            if target_feed == "ALL":
                # TR ve US varsayılan
                logger.info("TR USOM feed çekiliyor...")
                c1 = cert_feed_handler.fetch_tr_usom()
                if c1: text_parts.append(c1)
                
                logger.info("US CISA feed çekiliyor...")
                c2 = cert_feed_handler.fetch_us_cisa_kev()
                if c2: 
                    import json
                    text_parts.append(json.dumps(c2)) # JSON extractor will handle it or regex will find patterns
                ...
                logger.warning(f"Desteklenmeyen CERT kodu: {target_feed} (Şimdilik sadece TR, US, ALL)")
        ...
        if args.github_feed or args.github_feed_url or args.github_feed_preset:
            ...
            if args.github_feed:
                ...
                # Preset logic could be here or in FeedManager, but for now specific
                if args.github_feed_preset == "maltrail":
                     content = gh_handler.fetch_from_repo("stamparm/maltrail")
        ...
        if not text_parts:
            parser.error("En az bir giriş kaynağı belirtmelisiniz: -f, -u, --cert-feed (veya 'all'), --github-feed")
